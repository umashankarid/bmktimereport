  ...
  const isFrozen = (dateStr) => {
    return frozenDates.some(freeze => {
      ...
      if (reason.includes('Auto-frozen: Activity paid for')) {
        ...
        // Only apply freeze if it matches selected trainer (if any)
        const trainerToCheck = selectedTrainer?.name || currentTrainer?.name;
        if (trainerToCheck && autoFrozenTrainer !== trainerToCheck) {
          return false;
        }
      }
